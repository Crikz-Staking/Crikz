...
import { FolderPlus, Settings, Trash2, Tag, Image as ImageIcon, X, Edit3, Download, Lock, Gavel, Clock } from 'lucide-react';
...
export default function UserCollection({ dynamicColor }: { dynamicColor: string }) {
  ...
  const handleEndAuction = (nftContract: string, tokenId: bigint) => {
      endAuction({
  ...
  return (
    <div className="space-y-8 relative min-h-[600px]">
            ...
            <div className="glass-card p-6 rounded-3xl border border-white/10 bg-background-elevated">
                ...
                {myListings.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        {myListings.map(item => (
                            <div key={`${item.nftContract}-${item.tokenId}`} className="bg-[#0A0A0F] rounded-2xl border border-white/5 p-4">
                        ...
                        {myAuctions.map(auction => {
                            ...
                            return (
                                <div key={auction.id} className="bg-[#0A0A0F] rounded-2xl border border-white/5 p-4 relative overflow-hidden">
                                    ...
                                    {isEnded && (
                                        <button 
                                            onClick={() => handleEndAuction(auction.nftContract, auction.tokenId)}
        ...
        {showListModal && <ListingModal tokenId={showListModal.id} onClose={() => setShowListModal(null)} onSuccess={() => { refetch(); refreshMarket(); }} />}
    </div>
  );
}
