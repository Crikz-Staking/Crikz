...
export class InputProcessor {
  ...
  public process(input: string, knownConcepts: Record<string, AtomicConcept>, dappContext?: DAppContext): InputAnalysis {
    ...
    const rawTokens = lowerInput.replace(/[^\w\s]/gi, '').split(/\s+/).filter(w => w.length > 0 && !this.noiseWords.has(w));
    ...
    const grammar: GrammarStructure = {
      ...
      isQuestion: input.includes('?'), isImperative: false
    };
    ...
    // --- NEW: Calculate Verbosity Needed ---
    ...
    return {
      ...
      intent: this.classifyIntent(lowerInput, grammar, inputVector, specificityScore, safetyAnalysis.rating),
      ...
      verbosityNeeded // <--- Added to return
    };
  }

  // --- LOGIC: VERBOSITY CALCULATOR ---
  ...
  private classifyIntent(input: string, grammar: GrammarStructure, vector: Vector, specificity: number, safety: SafetyRating): IntentType {
  ...
  private calculateEmotionalWeight(input: string, keywords: AtomicConcept[]): number {
    ...
    keywords.forEach(k => { if (k.emotional_valence) weight += (k.emotional_valence * 0.1); });
