...
export default function NeuralDashboard({ isOpen, onClose }: NeuralDashboardProps) {
    const { brainStats, logs, isOwner } = useCrikzlingV3();
    ...
    // Auto-select newest log on open
    useEffect(() => {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-xl flex flex-col md:flex-row overflow-hidden">
            {/* Sidebar List */}
            <div className="w-full md:w-80 border-r border-white/10 flex flex-col bg-[#050508]">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar">
                    {logs.length === 0 ? (
                        <div className="p-8 text-center text-gray-600 text-xs">No activity recorded yet.</div>
                    ) : (
                        logs.map((log) => (
                            <button
                                key={log.id}
                            ...
                            >
                                <div className="min-w-0">
                                    ...
                                    <div className="text-xs text-gray-500 truncate mt-1">
                                        &rarr; {log.output.substring(0, 30)}...
                                    </div>
                                </div>
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto">
            <div className="grid grid-cols-3 gap-4">
                ...
                <StatCard label="Emotional Delta" value={log.emotionalShift.toFixed(2)} color="text-blue-400" />
            </div>
            ...
            <div className="glass-card p-6 rounded-2xl border border-white/10 bg-black/20">
                ...
                <div className="flex h-4 gap-1 rounded-full overflow-hidden">
                    {log.vectors.response.map((val, i) => (
                        <div key={i} className="h-full bg-primary-500/50" style={{ width: `${val * 100}%`, opacity: 0.2 + (val * 0.8) }} title={`Dim ${i}: ${val}`} />
                    ))}
                </div>
                            ...
                            <React.Fragment key={node}>
                                ...
                                >
                                    <div className="relative group">
                                        ...
                                        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 bg-black/90 border border-white/10 px-2 py-1 rounded text-[10px] text-white whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                                            {node}
                                        </div>
                                    </div>
                                </div>
