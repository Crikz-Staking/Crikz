...
// Ensure these imported files exist and are correct in your project
...
contract CrikzV2 is ERC20, ERC2771Context, Ownable, ReentrancyGuard {
    ...
    bool isDEXTrade = _isDEXTransaction(from, to);
        if (isDEXTrade) {
            ...
            // Standard ERC20 transfer (no fees)
            super._transfer(from, to, amount);
        }
    }
    ...
    function _updatePool() internal {
        uint256 timeElapsed = block.timestamp > rewardPool.lastUpdateTime ? block.timestamp - rewardPool.lastUpdateTime : 0;
